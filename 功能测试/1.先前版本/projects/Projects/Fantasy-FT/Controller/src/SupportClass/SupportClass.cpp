...
void SupportClass::createMem() {
  ...
  // shared->text = new char[10000];
  // shared->result = new char[10000];
}

/* 将 测试用例实体集 放入 共享内存中 */
void SupportClass::putTestCasesInMem(string file_name) {
  // 1.获取测试用例文件 并封装成实体
  this->ReadXmlFile(file_name);
  // this->ReadXmlFile("test.xml");
  shared->currentIndex = 1;
  // 2.保存在共享内存中
  // std::cout << "/* putTestCasesInMem--size*/" << this->showTestCaseList().size()
  //           << std::endl;
  ...
  // std::cout << "/* putTestCasesInMem */" << shared->text << std::endl;
}
