...
void Copter::failsafe_gcs_check()
{
    ...
    if (last_gcs_update_ms < FS_GCS_TIMEOUT_MS) { 
        ...
        if (failsafe.gcs) {
			// ------------------------  ²å×®µã ---------------------------------
			start = clock();
			supt->setCurProcessResult("failsafe_gcs_off_event", start, 1);
			// ------------------------  ²å×®¼¤Àø --------------------------------- 
			failsafe_gcs_off_event();
			end = clock();
			supt->setCurProcessResult("failsafe_gcs_off_event", end, 2);
			supt->setCurProcessResult("failsafe_gcs_off_event", (end - start), 3); 
			// ------------------------  ²å×®µã ---------------------------------
			start = clock();
			supt->setCurProcessResult("set_failsafe_gcs", start, 1);
			// ------------------------  ²å×®¼¤Àø --------------------------------- 
			set_failsafe_gcs(false);
			end = clock();
			supt->setCurProcessResult("set_failsafe_gcs", end, 2);
			supt->setCurProcessResult("set_failsafe_gcs", (end - start), 3);
        }
        return;
    }
    ...
    switch(control_mode) {
        ...
        case SPORT:
            ...
            if (ap.throttle_zero || ap.land_complete) {
				...
				// ------------------------  ²å×®µã ---------------------------------
				start = clock();
				supt->setCurProcessResult("set_mode_RTL_or_land_with_pause", start, 1);
				// ------------------------  ²å×®¼¤Àø --------------------------------- 
				set_mode_RTL_or_land_with_pause();
				end = clock();
				supt->setCurProcessResult("set_mode_RTL_or_land_with_pause", end, 2);
				...
				// ------------------------  ²å×®µã ---------------------------------
				start = clock();
				supt->setCurProcessResult("set_mode_land_with_pause", start, 1);
				// ------------------------  ²å×®¼¤Àø --------------------------------- 
				set_mode_land_with_pause();
				end = clock();
				supt->setCurProcessResult("set_mode_land_with_pause", end, 2);
