...
check_sample:
    if (!_hil_mode) {
        ...
        while (!gyro_available || !accel_available) {
			//cout << "before gyro_available :" << gyro_available << " accel_available : " << accel_available << endl;
            for (uint8_t i=0; i<_backend_count; i++) {
                gyro_available |= _backends[i]->gyro_sample_available();
                accel_available |= _backends[i]->accel_sample_available();
            }
			//cout << "after gyro_available :" << gyro_available << " accel_available : " << accel_available << endl;
            if (!gyro_available || !accel_available) {
                hal.scheduler->delay_microseconds(100);
            }
			//cout << "finally gyro_available :" << gyro_available << " accel_available : " << accel_available << endl;
        }
		cout << "9" << endl;
    }
	cout << "10" << endl;
