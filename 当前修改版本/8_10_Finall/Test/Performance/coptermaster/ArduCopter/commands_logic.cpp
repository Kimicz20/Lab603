...
void Copter::do_takeoff(const AP_Mission::Mission_Command& cmd)
{
    ...
    takeoff_alt = maxf(takeoff_alt,current_loc.alt);
    takeoff_alt = maxf(takeoff_alt,100.0f);
...
{
    if (millis() - condition_start > (uint32_t)maxf(condition_value,0)) {
        condition_value = 0;
        return true;
    }
    return false;
...
bool Copter::verify_within_distance()
{
    ...
    calc_wp_distance();
    if (wp_distance < maxf(condition_value,0)) {
        condition_value = 0;
        return true;
    }
    return false;
