...
void Copter::guided_run()
{
	...
	string filed[] = { "5","guided_takeoff_run", "guided_pos_control_run", "guided_vel_control_run", "guided_posvel_control_run", "guided_angle_control_run" };
	b = supt->getParamValueFormNamesWithKey(filed, "guided_mode");
...
{
	long start, end;
	cout << "---- guided_takeoff_run start ----" << endl;
...
#endif
		cout << "---- guided_takeoff_run end ----" << endl;
        return;
    }
	...
	//È±Ê§
	cout << "failsafe.radio" << failsafe.radio<< endl;
	if (failsafe.radio == 0) {
 //   if (!failsafe.radio) {
