...
void AP_InertialSensor::wait_for_sample(void)
{
    ...
    if (_next_sample_usec - now <=_sample_period_usec) {
		...
		supt->setCurProcessResult("delay_microseconds", start, 1);
		// ------------------------  插桩激励 --------------------------------- 
        
		hal.scheduler->delay_microseconds_boost(wait_usec);

		end = clock();
		//由于代码中没有SITL库所以直接插桩以下 激励
		supt->setCurProcessResult("wait_clock", (end - start), 3);
		supt->setCurProcessResult("_fdm_input_step", (end - start), 3);
		supt->setCurProcessResult("_fdm_input_local", (end - start), 3);
		supt->setCurProcessResult("_fdm_input", (end - start), 3);
		supt->setCurProcessResult("_simulator_output", (end - start), 3);
		supt->setCurProcessResult("_simulator_servos", (end - start), 3);
		
		supt->setCurProcessResult("delay_microseconds", end, 2);
		supt->setCurProcessResult("delay_microseconds", (end - start), 3);
