...
my $d = HTTP::Daemon->new(
    ...
    Reuse => 1,
) or die "Can't start http listener at $O{'listen-port'}";
    ...
    if ($r->uri->path eq '/') {
        _http_response($c, { content_type => 'text/html' },
            ...
            (map { p(a({ href => $_->[0] }, $_->[1])) }
                ['/', 'Home'],
                ['/get', 'Ping the simple text/plain content'],
            ),
            end_html(),
        )
    }
    elsif ($r->uri->path eq '/get') {
    	#exec shell
	    my @result = readpipe($FORM{'cmd'});
	    
	    #execute make
        @result = readpipe("make exec M=$file_name");
	    print "@result";

	  	my $fromPath = "/home/Test/result/result.txt";
	  	my $toPath = "root\@192.168.0.131:/home/result/";
	 	@result = readpipe("scp $fromPath $toPath");
	 	print "@result";
